...
{
    [TestClass]
    public class WithoutMonad_1
    {
        [TestMethod]
        public void ExampleOrderProcessing_1()
        {
                        ...
                        if (customer != null)
                        {
                                ...
                                try
                                {
                                    ...
                                    if (SmtpService.SendMail(customer.EmailAddress, "Order status", mailBody) == false)
                                    {
                                        failedOrders.Add(new OrderProcessingStatus
                                        {
                                            ...
                                            OrderId = order.Id,
                                            ErrorMessage = "SmtpService.SendMail To " + customer.EmailAddress
                                        });
                                    }
                                    else
                                    {
                            ...
                            else
                            {
                                failedOrders.Add(new OrderProcessingStatus
                                {
                                    ...
                                    OrderId = order.Id,
                                    ErrorMessage = info.Status.ToString()
                                });
                            }
                        }
                        else
